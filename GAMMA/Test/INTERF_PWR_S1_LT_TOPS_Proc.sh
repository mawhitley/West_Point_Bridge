...
if [ -e $demfile ]    #[  "$step" = "0" ]
then
	echo "-----------------INPUT DEM FILE - $demfile is exist--------------"
	echo "-----------will preparing look-up-table and sim_unw file---------"
...
then 
	...
	echo "offset_pwr ${mname}.slc ${sname}.rslc ${mname}.slc.par ${sname}.rslc.par ${ifgname}.off_${i} offs snr 256 64 offsets 1 64 256 10"
	offset_pwr ${mname}.slc ${sname}.rslc ${mname}.slc.par ${sname}.rslc.par ${ifgname}.off_${i} offs snr 256 64 offsets 1 64 256 10

	offset_fit offs snr ${ifgname}.off_${i} - - 10 1 > offsetfit${i}.log
...
while [ "$i" -le $numit ]
  ...
  echo "offset_fit offs snr ${ifgname}.off_${i} - - 10 1 > offsetfit${i}.log"
  offset_fit offs snr ${ifgname}.off_${i} - - 10 1 > offsetfit${i}.log
